...
mod tests {
    ...
    use crate::test_utils::{HACKATOM_WASM, IBC_REFLECT_WASM};
    use once_cell::sync::Lazy;

    // WASM artifacts are downloaded from the upstream repository when first
    // accessed. `Lazy` ensures each binary is fetched only once per test run.
    static HACKATOM: Lazy<&[u8]> = Lazy::new(|| HACKATOM_WASM.as_slice());
    static IBC_REFLECT: Lazy<&[u8]> = Lazy::new(|| IBC_REFLECT_WASM.as_slice());
