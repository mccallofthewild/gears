...
/// Fetches a WASM binary from the upstream repository at runtime.
pub fn fetch_remote_wasm(url: &str) -> Vec<u8> {
...
pub static CYBERPUNK_WASM: Lazy<Vec<u8>> = Lazy::new(|| {
    fetch_remote_wasm(
        "https://raw.githubusercontent.com/CosmWasm/wasmvm/main/testdata/cyberpunk.wasm",
    )
});

pub static HACKATOM_WASM: Lazy<Vec<u8>> = Lazy::new(|| {
    fetch_remote_wasm(
        "https://raw.githubusercontent.com/CosmWasm/wasmvm/main/testdata/hackatom.wasm",
    )
});

pub static IBC_REFLECT_WASM: Lazy<Vec<u8>> = Lazy::new(|| {
    fetch_remote_wasm(
        "https://raw.githubusercontent.com/CosmWasm/wasmvm/main/testdata/ibc_reflect.wasm",
    )
});
